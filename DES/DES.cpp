...
#include <ctime>
using namespace std;
using table = const unsigned char;

class DES {
public:
	DES(const bitset<64>& key) {
		// Generate subkeys by key.
		bitset<28> left, right;
		for (size_t i = 0; i < 28; i++) {
			left[i] = key[PC1L[i]-1];
			right[i] = key[PC1R[i]-1];
		}
	...
	bitset<64> Encrypt(const bitset<64>& plain) {
		...
		// Final permutation
		bitset<64> all(left.to_string() + right.to_string());
	...
	bitset<32> F(bitset<32>& R, const bitset<48>& subkey) {
		// TODO: Implement this function.
		return bitset<32>();
	}
...
private:
	...
	static constexpr const table LSCount[] = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
...
void encrypt(ofstream& fout) {
	...
	while (fin >> keyText >> plainText){
		...
		fout << hex << cipher.to_ullong() << endl;
	}
	fin.close();
}
